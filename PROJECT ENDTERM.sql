...
CREATE INDEX ind1 on tracking(tracking_code,timeline);
CREATE INDEX ind2 on customers(id);
CREATE INDEX ind3 on delivery_vehicle(tracking_code);
CREATE INDEX ind4 on orders(bar_code);
CREATE INDEX ind5 on purchases(tracking_code);
CREATE INDEX ind6 on credit_card_data(credit_card_number);
CREATE INDEX ind7 on vehicle(transported_via);
